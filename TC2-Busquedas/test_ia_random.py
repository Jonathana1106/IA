...
def choose_first():
    """
    Decide whether the computer or the computer makes the first move.

    Returns:
        str: 'Y' if computer goes first, 'N' if computer goes first.
    """
    first = ''
...
def render(state, pchoice, hchoice):
    ...
    Parameters:
        ...
        hchoice (str): The computer's choice ('X' or 'O').
    """
    chars = {
        -1: hchoice,
        +1: pchoice,
...
def announce_winner(p1_choice, p2_choice):
    ...
    """
    if gameover(board, PC):  # Check if Minimax AI wins
        print('PLAYER IA MINMAX WINS!')
    elif gameover(board, PC2):  # Check if Random AI wins
...
def iaturn1(p1_choice, p2_choice):
    """
    Executes the AI player's turn for Player 1 in the game.
