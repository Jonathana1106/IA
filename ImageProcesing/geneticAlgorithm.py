...
def tournament_selection(population, target_image, tournament_size):
    selected = []
    while len(selected) < tournament_size:
        # Randomly select indices of individuals for the tournament
        tournament_indices = np.random.choice(len(population), tournament_size, replace=False)
        
        # Evaluate their fitness and select the best individual
        tournament_individuals = [population[i] for i in tournament_indices]
        fitness_values = [fitness(individual, target_image) for individual in tournament_individuals]
        winner = tournament_individuals[np.argmin(fitness_values)]
...
def genetic_algorithm(population, target_image, generations, tournament_size, mutation_rate):
...
def main():
    ...
    best_image = genetic_algorithm(population, flattened_enhanced_image, generations, tournament_size, mutation_rate)
